...
import { Modal, Button, Form, Space, Select, Input, Tag, Popover, Row, Typography } from 'antd';
...
class AddPublicBodiesSegmentModal extends Component {
    ...
    onOK = () => {
        ...
        if (segmentName.length !== 0 && ngoType.length !== 0) {
            ...
            const selected_ngo_types = this.props.categories.customer_segments_types.ngo_types.filter((item) => ngoType.some((field) => item.id === field));
            const reducerObj = {
    ...
    onNgoTypeChange(value) {
        const ngo_type_value = this.addSelectedValue(value, this.state.ngoType, this.props.categories.customer_segments_types.ngo_types);
        this.setState({
            ngoType: ngo_type_value,
